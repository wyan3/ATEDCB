...
#' @return ATE
#'
#' @examples ATE.DCB_V2(Y,T,X)
#'
#' @export ATE.DCB_V2

ATE.DCB_V2 <- function(Y,T,X,ATT=FALSE,lambda=10,delta=0.001,mu=0.001,upsilon=0.001,thold=1e-4,max_iter=100000){
  if(ATT){
    ...
    fit1 <- ATT.DCB(Y,T,X,gp=ATT,lambda=lambda,delta=delta,mu=mu,upsilon=upsilon,thold=thold,max_iter=max_iter)
    fit2 <- ATT.DCB(Y,1-T,X,gp=ATT,lambda=lambda,delta=delta,mu=mu,upsilon=upsilon,thold=thold,max_iter=max_iter)
    wt <- rep(NA,length(T))
    wt[T==0] <- fit1$weight
    wt[T==1] <- fit2$weight
